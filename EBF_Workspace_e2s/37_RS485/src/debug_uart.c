...
void Debug_UART_Callback(uart_callback_args_t * p_args)
{
    switch(p_args->event)
    {
        case UART_EVENT_RX_CHAR:  //收到数据后发给RS485_1
            R_SCI_UART_Write(RS485_1.p_ctrl, (uint8_t *)&p_args->data, 1);
            break;
...
//重定向printf输出
 ...
 int _write(int fd, char *pBuffer, int size)
 {
    ...
    R_SCI_UART_Write(Debug_UART.p_ctrl, (uint8_t *)pBuffer, (uint32_t)size);
 ...
 int fputc(int ch, FILE *f)
 {
    (void)f;
    uart_send_complete_flag = false;
    R_SCI_UART_Write(Debug_UART.p_ctrl, (uint8_t *)&ch, 1);
    while(uart_send_complete_flag == false);
    uart_send_complete_flag = false;
    return ch;
 }
